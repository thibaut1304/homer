...
# -------- production stage with Node + Homer --------
FROM node:22-alpine3.21 AS prod-stage
...
# RUN addgroup -S homer -g ${GID} && adduser -D -S -u ${UID} homer homer
...
COPY server-package.json /www/package.json
